...
export default function CreateProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        console.error("Supabase upsert error:", error) // Log the full error object for debugging
        throw error // Re-throw to be caught by the outer catch
      }

      router.push("/dashboard")
    } catch (error: any) {
      console.error("Error creating profile:", error) // Log the error caught here
  ...
  if (loading && user) {
    // Only show loading if user is present and we're checking profile
    return (
  ...
  if (!user) {
    return null // Or a redirect to login, handled by useEffect
  }
